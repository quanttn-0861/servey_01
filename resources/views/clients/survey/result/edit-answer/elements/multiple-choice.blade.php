            ...
            @if ($answer->type == config('settings.answer_type.other_option'))
                <label class="container-radio-setting-survey">@lang('lang.other')
                    {!! Form::radio(
                        'answer' . $question->id,
                        '',
                        $result && $result->answer_id == $answer->id ? true : false,
                ...
                <div class="magic-box-preview">
                    {!! Form::text('',
                        $result && $result->answer_id ==  $answer->id ? $result->content : '',
                        ['class' => 'option-other input-answer-other input-multiple-choice-other']
                    ) !!}
                </div>
            @else
